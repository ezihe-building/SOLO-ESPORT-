import { Router, type IRouter, type Request, type Response } from "express";
  ...
  router.post("/mgmt/announcements", async (req, res): Promise<void> => {
    ...
    try {
      ...
      const [created] = await db.insert(announcementsTable).values({
        ...
        imageUrl: imageUrl ?? null, linkUrl: linkUrl ?? null, linkLabel: linkLabel ?? null,
      }).returning();
  ...
  router.patch("/mgmt/announcements/:id", async (req, res): Promise<void> => {
    ...
    try {
      ...
      const [updated] = await db.update(announcementsTable).set(u as any).where(eq(announcementsTable.id, id)).returning();
  ...
  router.post("/mgmt/events", async (req, res): Promise<void> => {
    ...
    try {
      ...
      const [created] = await db.insert(eventsTable).values({
        ...
        imageUrl: imageUrl ?? null, linkUrl: linkUrl ?? null, linkLabel: linkLabel ?? null,
        eventDate: new Date(eventDate), createdBy: "owner", createdByName: "Owner",
      }).returning();
  ...
  router.patch("/mgmt/events/:id", async (req, res): Promise<void> => {
    ...
    try {
      ...
      const [updated] = await db.update(eventsTable).set(u as any).where(eq(eventsTable.id, id)).returning();
      if (!updated) { res.status(404).json({ error: "Not found" }); return; }
  ...
  router.post("/mgmt/feed", async (req, res): Promise<void> => {
    ...
    try {
      ...
      const [created] = await db.insert(feedPostsTable).values({
        id: randomUUID(), authorId: "owner", authorName: "Owner", authorRole: "OWNER",
        content, imageUrl: imageUrl ?? null, postType: (postType ?? "news") as any, likeCount: 0,
      }).returning();
      res.status(201).json({ ...created, createdAt: created.createdAt.toISOString() });
    } catch (err) { dbErr(res, err); }
  });
  ...
  router.post("/mgmt/scrims", async (req, res): Promise<void> => {
    ...
    try {
      ...
      const [created] = await db.insert(scrimsTable).values({
        id: randomUUID(), opponentName,
        scheduledAt: new Date(scheduledAt),
        gameMode: gameMode ?? "Battle Royale",
        requiredPlayers: requiredPlayers ?? 5,
        notes: notes ?? null, imageUrl: imageUrl ?? null,
        linkUrl: linkUrl ?? null, linkLabel: linkLabel ?? null,
        status: "upcoming", createdBy: "owner",
      }).returning();
  ...
  router.patch("/mgmt/scrims/:id", async (req, res): Promise<void> => {
    ...
    try {
      ...
      const [updated] = await db.update(scrimsTable).set(u as any).where(eq(scrimsTable.id, id)).returning();
      if (!updated) { res.status(404).json({ error: "Not found" }); return; }
